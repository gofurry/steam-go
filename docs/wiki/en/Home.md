# steam-go Wiki

### [中文版本 Chinese Version](https://github.com/gofurry/steam-go/wiki/%E9%A6%96%E9%A1%B5)

Welcome to the `steam-go` Wiki.

`steam-go` is a lightweight Go SDK for the Steam Web API, with practical toolkit features for real-world usage, including client configuration, credential management, retry and rate limiting, proxy selection, traffic policies, OpenID login verification, A2S server queries, error handling, and safe URL redaction.

The repository now also includes a small read-only `client.Web.*` layer for selected Storefront, Community, and Market JSON endpoints outside `api.steampowered.com`.

This Wiki focuses on helping developers quickly understand how the SDK is organized, what each toolkit feature is used for, and how to apply them with small, practical examples.

## Start Here

- [Quick Reference](https://github.com/gofurry/steam-go/wiki/Quick-Reference)
- [速查手册](https://github.com/gofurry/steam-go/wiki/%E9%80%9F%E6%9F%A5%E6%89%8B%E5%86%8C)

## SDK Toolkit

- [Credential Model](https://github.com/gofurry/steam-go/wiki/Credential-Model)
- [Rate Limiting Strategy](https://github.com/gofurry/steam-go/wiki/Rate-Limiting-Strategy)
- [Proxy and Network Strategy](https://github.com/gofurry/steam-go/wiki/Proxy-and-Network-Strategy)
- [Traffic Policy](https://github.com/gofurry/steam-go/wiki/Traffic-Policy)
- [Error Handling](https://github.com/gofurry/steam-go/wiki/Error-Handling)

## Steam Notes

- [Steam Web API Notes](https://github.com/gofurry/steam-go/wiki/Steam-Web-API-Notes)
- [Public Store Page Access Notes](https://github.com/gofurry/steam-go/wiki/Public-Store-Page-Access-Notes)
- [Partner API Notes](https://github.com/gofurry/steam-go/wiki/Partner-API-Notes)
- [OpenID Notes](https://github.com/gofurry/steam-go/wiki/OpenID-Notes)
- [A2S Notes](https://github.com/gofurry/steam-go/wiki/A2S-Notes)
- [Steam Keys and Access Tokens](https://github.com/gofurry/steam-go/wiki/Steam-Keys-and-Access-Tokens)
- [Steam Static Assets](https://github.com/gofurry/steam-go/wiki/Steam-Static-Assets)
- [Steam VDF and addons/vdf](https://github.com/gofurry/steam-go/wiki/Steam-VDF-and-addons-vdf)

## Design Notes

- [Why Safe Defaults](https://github.com/gofurry/steam-go/wiki/Why-Safe-Defaults)
- [Why Traffic Classes](https://github.com/gofurry/steam-go/wiki/Why-Traffic-Classes)
- [Why ProxySelector](https://github.com/gofurry/steam-go/wiki/Why-ProxySelector)

## Recommended Start

If you are new to this project, start with the Quick Reference page.

It gives a compact overview of the toolkit layer without diving into every wrapped Steam business API.
